 ...
 */
__INLINE__
int32_t Kernel_InjFifoActivate (MUSPI_InjFifoSubGroup_t * subgrp,
				uint32_t                  nfifos,
				uint32_t                * fifoids,
				uint32_t                  activate)
{
  ...
  sgid = subgrp->subgroupid % BGQ_MU_NUM_FIFO_SUBGROUPS;
  ...
  {
    subgrp->groupStatus->enable  = bits;
/*     printf("Storing 0x%llx at %p for enable\n", */
/* 	   (unsigned long long int)(bit), */
/* 	   &(subgrp->groupStatus->enable)); */
  }
  else
    subgrp->groupStatus->disable = bits;
